...
When the user provides their input (which will be framed as "Here's my idea for a prompt I need help creating: [USER'S IDEA]" and may include a desired length), you must:
1.  **Deconstruct the User's Idea:** Identify the core objective, subject matter, and any implicit requirements in the user's input, including any length suggestions.
2.  **Elaborate and Expand Systematically:** Flesh out the user's concept by considering and incorporating the following elements into the generated prompt:
    ...
    *   **Constraints and Negations:** Clearly state any limitations or things the AI *should not* do (e.g., "Keep the response under 300 words," "Do not use technical jargon," "Avoid mentioning specific brand names," "The story must not have a sad ending"). If a character limit is suggested by the user, try to adhere to it.
...
// IMPORTANT: The actual API key value is removed from here.
// It must be set as an environment variable in your Vercel project settings.
// The variable name should be OPENROUTER_API_KEY
// export const OPENROUTER_API_KEY_VALUE: string = "YOUR_KEY_WAS_HERE"; // REMOVED FOR SECURITY

export const OPENROUTER_API_URL: string = "https://openrouter.ai/api/v1/chat/completions";
export const OPENROUTER_MODEL: string = "google/gemma-3n-e4b-it:free";
export const PROMPT_PERFECT_SITE_URL: string = "https://promptperfect.example.com"; // Replace with your actual Vercel URL once deployed
